...
export function MapCircle({ title }: { title: string }) {
  ...
  /* Mapa je cizí dokument — načtením by Google dostal IP i cookies. Dokud
     souhlas neznáme nebo ho nemáme, nabídneme místo ní tlačítko. */
  if (consent?.embeds !== true) {
    return (
      <div className="media media--circle map-circle map-circle--off">
        <div className="map-circle__ask">
          <p>Mapu načteme, až budete chtít — je od Googlu.</p>
          <button
            type="button"
            className="btn btn--sm"
            onClick={() => writeConsent({ ...(readConsent() ?? { analytics: false, embeds: false }), embeds: true })}
          >
            Zobrazit mapu
          </button>
        </div>
