...
Schema.org JSON-LD is generated by `scripts/generate-schema.js` from public static HTML pages and `data/schema-business.json`.

Do not edit page-level `<script type="application/ld+json">` blocks manually. Run:

```text
npm run generate:schema
```

When business facts change, update `data/schema-business.json` first, then regenerate schema. The generator must include the shared Visibly business entity, WebSite, WebPage, BreadcrumbList, and page-specific entities such as Service, Article, ContactPage, or CreativeWork where relevant.

After creating a new public page, run:
